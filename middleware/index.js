...
app.use((req,res,next) => {
    ...
    // now call to the next middleware if exist or call the route handler if no more middleware exist
    req.myusername = "Aditya"
    next();
});

//property of middleware 1 is access by middleware 2 and route handler
app.use((req,res,next) => {
...
app.get("/api/users", (req, res) => { //give json data
    console.log("hello from route handler", req.myusername);
    res.json(users);
});
